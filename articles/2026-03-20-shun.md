...
`search_mode = "migemo"` にすると、ローマ字で日本語のファイル名や候補を検索できます。たとえば `hajime` と入力すると `初めてのRust` や `はじめに` にマッチします。日本語ファイル名のメモやドキュメントを大量に扱う場合に特に便利です。
...
```toml
# 検索モードとソート順
search_mode = "fuzzy"       # "fuzzy" | "exact" | "migemo"
